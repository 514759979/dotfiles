...
class CustomApplications(Applications):
	def app_default(self, c):
		...
		if f.extension is not None:
			...
			if f.extension in ('html', 'htm', 'xhtml') or f.extension == 'swf':
				...
				handler = self.either(c,
						'o', 'luakit', 'uzbl', 'vimprobable', 'vimprobable2', 'jumanji',
			...
			if f.extension == 'doc':
				return self.either(c, 'o', 'abiword', 'libreoffice',
						'soffice', 'ooffice')
			if f.extension in ('odt', 'ods', 'odp', 'odf', 'odg', 'odb', 'sxc',
					'stc', 'xls', 'xlsx', 'xlt', 'xlw', 'gnm', 'gnumeric'):
				return self.either(c, 'o', 'gnumeric', 'kspread',
		...
		if f.video or f.audio:
			if f.video:
				c.flags += 'd'
			return self.either(c, 'loop_play', 'mplayer', 'gmplayer', 'mplayer2',
					'smplayer', 'vlc', 'totem')
		...
		if f.image:
			if c.mode in (11, 12, 13, 14):
				...
				return self.either(c, 'o', 'sxiv', 'feh', 'eog', 'mirage')
